...
function Data (options) {
  ...
  this.link = function (collection, entry) {
    // Error checking for linkage that already exists
    ...
    // Check for duplicate link entry
    const exists = data[collection].some(existing =>
      Object.keys(entry).every(key => existing[key] === entry[key])
    )
    if (exists) {
      return makeReturnValue(this.StatusEnum.ERROR, 'Linkage already exists in ' + collection)
    }
  ...
  const fsOp = function (dir, callback) {
    ...
    try {
      ...
      return makeReturnValue(this.StatusEnum.ERROR, e.message)
    }
    return makeReturnValue(this.StatusEnum.OK, '')
  }
